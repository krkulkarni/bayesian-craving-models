...
class RWDecay(object):
    ...
    def _theano_llik(self, _alpha, _decay, _beta, _actions, _rewards):
        ...
        Qs, updates = theano.scan(
            ...
            sequences=[act, rw],
            outputs_info=[Qs],
            non_sequences=[_alpha, _decay, self.n_blocks],
        )
    ...
    def fit(self, draws=500, chains=2, cores=4, jupyter=False, rerun=False):
        ...
        #fmt: on
            ...
            with pm.Model() as m:
                ...
                decay = pm.Normal(f"decay", 0, 2, shape=self.n_blocks)
                ...
                self.traces[pid] = pm.sample(start={'alpha': np.array([0.5, 0.5]), 'decay': np.array([0, 0]), 'beta': np.array([3, 3])},
    ...
    def calc_Q_table(self):
        ...
        for pid in self.summary['PID']:
            ...
            mean_decays = self.traces[pid].posterior.decay.to_numpy().mean(axis=1).mean(axis=0)
