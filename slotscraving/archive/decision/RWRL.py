...
class RWRL(object):
    ...
    def update_func(self, action, reward, Qs, alpha_pos, alpha_neg, n_blocks):
        ...
        Args:
            ...
            alpha_pos (theano.tensor.fvector): Theano vector of alpha_pos for each block
            alpha_neg (theano.tensor.fvector): Theano vector of alpha_neg for each block
        ...
        block_alphas = tt.stack(alpha_pos, alpha_neg).T[tt.arange(2), tt.cast(PE < 0, 'int16')]
        Qs = tt.set_subtensor(
            Qs[tt.arange(n_blocks), action],
            Qs[tt.arange(n_blocks), action] + block_alphas * PE,
        )
        return Qs
    ...
    def _theano_llik(self, _alpha_pos, _alpha_neg, _beta, _actions, _rewards):
        ...
        Qs, updates = theano.scan(
            ...
            sequences=[act, rw],
            outputs_info=[Qs],
            non_sequences=[_alpha_pos, _alpha_neg, self.n_blocks],
        )
    ...
    def fit(self, draws=500, chains=2, cores=4, jupyter=False, rerun=False):
        ...
        #fmt: on
            ...
            with pm.Model() as m:
                alpha_pos = pm.Normal(f"alpha_pos", 0.5, 0.25, shape=self.n_blocks)
                alpha_neg = pm.Normal(f"alpha_neg", 0.5, 0.25, shape=self.n_blocks)
                ...
                self.traces[pid] = pm.sample(start={'alpha_pos': np.array([0.5, 0.5]), 'alpha_neg': np.array([0.5, 0.5]), 'beta': np.array([3, 3])},
    ...
    def calc_Q_table(self):
        ...
        for pid in self.summary['PID']:
            ...
            mean_alphas_pos = self.traces[pid].posterior.alpha_pos.to_numpy().mean(axis=1).mean(axis=0)
            mean_alphas_neg = self.traces[pid].posterior.alpha_neg.to_numpy().mean(axis=1).mean(axis=0)
