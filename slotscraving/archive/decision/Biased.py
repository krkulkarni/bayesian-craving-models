...
class Biased(object):
    def __init__(self, model_name, save_path, summary=None, longform=None):
        ...
        netcdf model traces are available. Likely DOES NOT NEED TO GET MODIFIED beyond model name and n_params.
    ...
    def _theano_llik(self, _bias, _actions):
        ...
        Args:
            _bias (theano.tensor.fvector): Theano vector of biases for each block
        ...
        """
        act = tt.cast(_actions.T, 'int16').eval()
        bias = tt.cast(_bias, 'float64')
        return tt.sum(
            np.log(
                (act==1)*bias.reshape(-1,1) + 
                (act==0)*(1-bias).reshape(-1,1)
            )
        )
    ...
    def fit(self, draws=500, chains=2, cores=4, jupyter=False, rerun=False):
        ...
        #fmt: on
            ...
            with pm.Model() as m:
                bias = pm.Normal(f"bias", 0.5, 0.2, shape=self.n_blocks)
                like = pm.DensityDist('like', self._theano_llik, observed=dict(_bias=bias, _actions=actions))
                self.traces[pid] = pm.sample(start={'bias': np.array([0.5, 0.5])},
    ...
    def calc_Q_table(self):
        ...
        for pid in self.traces.keys():

            ## CALCULATE MEANS OF PARAMS

            mean_bias = self.traces[pid].posterior.bias.to_numpy().mean(axis=1).mean(axis=0)
            ...
            ## UPDATE VALUES - no values calculated, just leave to default
    ...
    def calc_bics(self):
        ...
        for pid in self.summary['PID']:
            ...
            for bias_chain in self.traces[pid].posterior.bias.values:
                for block_params in bias_chain:
                    sample_bic = []
                    for b, block in enumerate(['money', 'other']):
                        block_param = block_params[b]
                        block_actions = actions[b]
                        block_rewards = rewards[b]
                        log_prob_actions = np.zeros(len(block_actions))
