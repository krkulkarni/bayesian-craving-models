...
class RPE2sCue(Prototype):
    def __init__(
        ...
        ##------- MODEL PARAMETERS -------##
        self.craving_model_name = "RPE2sCue"
        self.equation = "w0 + w1 * PE + w2 * cueelic"
        self.n_params = 4
        ##--------------------------------##
        ###############################################################################
            ...
            for i, ind in enumerate(craving_inds):
                if ind < 2:
                    pe_reg = tt.set_subtensor(
                        pe_reg[:, :, i], pes[:, :, ind] + pes[:, :, ind - 1]
                    )
                else:
                    pe_reg = tt.set_subtensor(
                        pe_reg[:, :, i],
                        pes[:, :, ind] + pes[:, :, ind - 1] + pes[:, :, ind - 2],
                    )
                cueelic_reg = tt.set_subtensor(
                    cueelic_reg[:, :, i],
                    np.repeat([rewards[:, ind]], self.n_samples, axis=0).T,
                )
            pred_craving = w0_ + w1_ * pe_reg + w2_ * cueelic_reg
    ...
    def get_sample_predictions(
        ...
        for samp, (w0, w1, w2) in enumerate(zip(w0_samples, w1_samples, w2_samples)):
            ...
            # cueelic_reg = np.zeros((self.n_blocks, norm_craving_ratings.shape[2]))

            for b, block in enumerate(["money", "other"]):
                block_pes = pes[b, samp, :]
                for i, ind in enumerate(craving_inds):
                    if ind < 2:
                        pes_reg[b, i] = block_pes[ind] + block_pes[ind - 1]
                    else:
                        pes_reg[b, i] = (
                            block_pes[ind] + block_pes[ind - 1] + block_pes[ind - 2]
                        )

            cueelic_reg = rewards[:, craving_inds]
            pred = w0_ + w1_ * pes_reg + w2_ * cueelic_reg
