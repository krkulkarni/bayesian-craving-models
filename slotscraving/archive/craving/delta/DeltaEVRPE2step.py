...
class DeltaEVRPE2step(object):
    def __init__(
        ...
        ##------- MODEL PARAMETERS -------##
        self.craving_model_name = "DeltaEVRPE2step"
    ...
    def get_sample_predictions(
        ...
        for samp, (w0, w1, w2) in enumerate(zip(w0_samples, w1_samples, w2_samples)):
            ...
            for b, block in enumerate(["money", "other"]):
                ...
                for i, ind in enumerate(craving_inds):
                    if ind < 2:
                        qs_reg[b, i] = block_qs[ind] + block_qs[ind - 1]
                        pes_reg[b, i] = block_pes[ind] + block_pes[ind - 1]
                    else:
                        qs_reg[b, i] = (
                            block_qs[ind] + block_qs[ind - 1] + block_qs[ind - 2]
                        )
                        pes_reg[b, i] = (
