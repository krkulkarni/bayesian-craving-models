...
class EVRPE2step(Prototype):
    def __init__(
        ...
        ##------- MODEL PARAMETERS -------##
        self.craving_model_name = "EVRPE2step"
            ...
            # fmt: on
                ...
                try:
                    with pm.Model() as model:
                        ...
                        w2_mean = pm.Normal('w2_mean', mu=0, sigma=1)
                        ...
                        w2 = pm.Normal('w2', mu=w2_mean, sigma=0.5, shape=self.n_samples)
                        ...
                        q_reg = tt.zeros((self.n_samples, norm_craving_ratings.shape[1]))
                        pe_reg = tt.zeros((self.n_samples, norm_craving_ratings.shape[1]))

                        for i, ind in enumerate(craving_inds):
                            if ind < 2:
                                q_reg = tt.set_subtensor(q_reg[:, i], qs[:, ind] + qs[:, ind - 1])
                                pe_reg = tt.set_subtensor(pe_reg[:, i], pes[:, ind] + pes[:, ind - 1])
                            else:
                                q_reg = tt.set_subtensor(q_reg[:, i], qs[:, ind] + qs[:, ind - 1] + qs[:, ind - 2])
                                pe_reg = tt.set_subtensor(pe_reg[:, i], pes[:, ind] + pes[:, ind - 1] + pes[:, ind - 2])
                        pred_craving = w0 + w1 * q_reg.T - w2 * pe_reg.T
                        pred = pm.Normal(
                            "pred",
                            mu=pred_craving.T,
                            sigma=craving_sig,
    ...
    def get_sample_predictions(
        ...
        try:
            ...
            w2_samples = np.hstack(
                [
                    self.traces[pid][block].posterior.w2_mean.values[0, :],
                    self.traces[pid][block].posterior.w2_mean.values[1, :],
                ]
            )
        ...
        for samp, (w0, w1, w2) in enumerate(zip(w0_samples, w1_samples, w2_samples)):
            ...
            qs_reg = np.zeros(norm_craving_ratings.shape[1])
            ...
            for i, ind in enumerate(craving_inds):
                if ind < 2:
                    qs_reg[i] = qs_[ind] + qs_[ind - 1]
                    pes_reg[i] = pes_[ind] + pes_[ind - 1]
                else:
                    qs_reg[i] = qs_[ind] + qs_[ind - 1] + qs_[ind - 2]
                    pes_reg[i] = pes_[ind] + pes_[ind - 1] + pes_[ind - 2]
            pred = w0 + w1 * qs_reg - w2 * pes_reg
