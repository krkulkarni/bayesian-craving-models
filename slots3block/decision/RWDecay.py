...
class RWDecay(Prototypes.DecisionModelPrototype):
    def __init__(self):
        super().__init__()
        self.params = {
            "alpha": "Learning rate",
            "decay": "Rate at which unchosen value decays to center",
    ...
    def _llik(self, x, *args):
        ...
        for t, (a, r) in enumerate(zip(actions[:-1], rewards[:-1])):
            ...
            Qs[t + 1, 1 - a] = Qs[t, 1 - a] + decay * (center_val - Qs[t, 1 - a])
...
class RWDecay_SimActRew(Prototypes.SimPrototype):
    def __init__(self):
        super().__init__()
        self.params = {
            "alpha": "Learning rate",
    ...
    def _update_values(
        self, trial_num, block, values, actions, rewards, param_set, internal_params
    ):
        ...
        values[trial_num + 1, trial_action] = values[
            trial_num, trial_action
        ] + param_set[block]["alpha"] * (rpe)
        values[trial_num + 1, 1 - trial_action] = values[
            trial_num, 1 - trial_action
        ] + param_set[block]["decay"] * (
            center_val - values[trial_num, 1 - trial_action]
        )
    ...
    def _select_action(self, trial_num, block, values, actions, param_set, beta=None):
        ...
        p = np.exp(chosen_beta * trial_value) / np.sum(
            np.exp(chosen_beta * trial_value)
        )
